...
import { AlertTriangle, Badge, Button, Card, CardContent, CardDescription, CardHeader, CardTitle, Dialog, DialogContent, DialogHeader, DialogTitle, DialogDescription, Input, Label, toast } from "@repo/ui";
...
function SettingsEmailSecretPanel({ canUpdate, enabled, selectedProvider }: { canUpdate: boolean; enabled?: unknown; selectedProvider?: unknown }) {
  ...
  const provider = selectedProvider === "ses" || selectedProvider === "resend"
    ? selectedProvider
    : data?.provider ?? "resend";
  const activeProviderConfigured = provider === "ses" || Boolean(data?.resendApiKeyConfigured);
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>{provider === "ses" ? "Amazon SES" : "Resend secret"}</CardTitle>
        <CardDescription>
          {provider === "ses"
            ? "SES uses the AWS credential and region configuration available to the server runtime."
      ...
      <CardContent className="space-y-5">
        {provider === "resend" ? (
          ...
          <div className="rounded-lg border border-border p-4">
            <div className="flex items-center justify-between gap-4">
              <div>
                <p className="text-sm font-medium">Runtime AWS configuration</p>
                <p className="mt-1 text-xs text-muted-foreground">
                  Configure SES through the server AWS profile, IAM role, or AWS environment variables.
                </p>
              </div>
        ...
        <div className="space-y-2">
          ...
          <div className="flex flex-col gap-2 sm:flex-row">
            ...
            <Button
              type="button"
              disabled={!canUpdate || sendTest.isPending || !testRecipient || !data?.enabled || !activeProviderConfigured}
          ...
          {(!data?.enabled || !activeProviderConfigured) && (
            <p className="text-xs text-muted-foreground">
              Enable email delivery, save the Settings form, and configure the selected provider credentials before sending a test.
            </p>
          )}
        </div>
      </CardContent>
...
function KeyValueResourceInner({ config, m, permissions, isProtectedRole }: { config: CRUDConfig; m: any; permissions: string[]; isProtectedRole: boolean }) {
  const { data, isLoading, refetch } = m.get.useQuery();
        ...
        extraTabContent={config.model === "settings" ? {
          Email: (values) => (
            <SettingsEmailSecretPanel
              ...
              selectedProvider={values.emailProvider}
            />
          ),
        } : undefined}
        onSave={async (values) => {
...
export function CRUDResourceClient({ config, permissions, isProtectedRole, currentUserEmail }: CRUDResourceClientProps) {
  ...
  if (config.mode === "keyValue") {
    return <KeyValueResourceInner config={config} m={m} permissions={permissions} isProtectedRole={isProtectedRole} />;
  }
