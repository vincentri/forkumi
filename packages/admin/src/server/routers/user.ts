  ...
  const userListProcedure = trpc.permissionProcedure("view", "user")
    ...
    .query(async ({ input }: { input: any }) => {
      ...
      const [rawItems, total] = await Promise.all([
        db.user.findMany({ where, skip, take: pageSize, orderBy, select: { id: true, name: true, email: true, emailVerified: true, image: true, password: true, roleId: true, createdAt: true, updatedAt: true, role: { select: { id: true, name: true, protected: true } } } }),
        db.user.count({ where }),
      ]);
