...
function getProgramInfo(gl: WebGLRenderingContext, program: WebGLProgram): ProgramInfo | null {
  ...
  if (!angle || !cellSize || !containerSize || !devicePixelRatio || !lineColor || !time || !viewportSize) {
    return null
  }
...
export function RetroGrid({
  ...
  return (
    <div
      ref={containerRef}
      className={`pointer-events-none absolute size-full overflow-hidden ${className || ""}`}
      style={gridStyles}
      {...props}
    >
      ...
      <canvas ref={canvasRef} className={`absolute inset-0 size-full ${isWebGlReady ? "opacity-100" : "opacity-0"}`} />
